...
export const AppReviewPanel: React.FC<AppReviewPanelProps> = ({
  updateService,
}) => {
  ...
  const loadReviewStats = () => {
    ...
    const installDate = localStorage.getItem(
      'capacitor-native-update-install-date'
    );
    const launchCount = localStorage.getItem(
      'capacitor-native-update-launch-count'
    );
    const lastReview = localStorage.getItem(
      'capacitor-native-update-last-review'
    );

    if (installDate) {
      const daysSinceInstall = Math.floor(
        (Date.now() - parseInt(installDate)) / (1000 * 60 * 60 * 24)
      );
    ...
    if (lastReview) {
      const daysAgo = Math.floor(
        (Date.now() - parseInt(lastReview)) / (1000 * 60 * 60 * 24)
      );
  ...
  const requestReview = async () => {
    ...
    try {
      ...
      if (shown) {
        setMessage({
          type: 'success',
          text: 'Review dialog was shown to the user',
        });
        // Update stats
        ...
        setMessage({
          type: 'info',
          text: 'Review dialog was not shown (platform restrictions or conditions not met)',
        });
      }
    } catch (error) {
  ...
  const simulateUserActions = () => {
    ...
    const currentCount = parseInt(
      localStorage.getItem('capacitor-native-update-launch-count') || '0'
    );
  ...
  return (
    <div className='panel'>
      ...
      <div className='panel-section'>
        ...
        {isChecking ? (
          ...
          <div className='info-group'>
            ...
            <span
              className={`status-badge ${canRequest.allowed ? 'active' : 'failed'}`}
            >
      ...
      <div className='panel-section'>
        ...
        <div className='alert info'>
          <p>
            <strong>iOS:</strong> Limited to 3 review requests per year per
            user. Uses StoreKit framework.
          </p>
          <p>
            <strong>Android:</strong> Uses Google Play In-App Review API. Has
            internal quotas managed by Google.
          </p>
          <p>
            <strong>Web:</strong> Shows a custom dialog as fallback.
          </p>
        </div>
