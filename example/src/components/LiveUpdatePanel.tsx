...
import type { BundleInfo, LatestVersion, SyncResult } from 'capacitor-native-update';

interface LiveUpdatePanelProps {
  updateService: UpdateService;
}

export const LiveUpdatePanel: React.FC<LiveUpdatePanelProps> = ({ updateService }) => {
  ...
  const checkForUpdates = async () => {
    ...
    try {
      ...
      if (result.status === 'UPDATE_AVAILABLE') {
        ...
        setMessage({ type: 'info', text: `Update available: v${result.version}` });
      } else if (result.status === 'UP_TO_DATE') {
        setMessage({ type: 'success', text: 'You have the latest version' });
      } else {
        setMessage({ type: 'error', text: result.error?.message || 'Update check failed' });
      }
    } catch (error) {
  ...
  const applyBundle = async (bundle: BundleInfo) => {
    try {
      await updateService.applyUpdate(bundle);
      setMessage({ type: 'success', text: 'Bundle applied. Reload to see changes.' });
      await loadBundleInfo();
    } catch (error) {
  ...
  const resetToOriginal = async () => {
    if (!confirm('Reset to original bundle? This will delete all updates.')) return;
    ...
    <div className="panel">
      ...
      <div className="panel-section">
        ...
        <div className="info-group">
          ...
          <span className="info-value">{currentBundle?.version || 'Unknown'}</span>
        </div>
        <div className="info-group">
          <span className="info-label">Bundle ID:</span>
          <span className="info-value">{currentBundle?.bundleId || 'Unknown'}</span>
        </div>
        <div className="info-group">
          <span className="info-label">Status:</span>
          <span className={`status-badge ${currentBundle?.status.toLowerCase()}`}>
      ...
      <div className="panel-section">
        ...
        <div className="channel-selector">
          <select value={selectedChannel} onChange={(e) => changeChannel(e.target.value)}>
        ...
        <div className="button-group">
          ...
          {latestVersion?.available && (
            ...
            >
              {isDownloading ? 'Downloading...' : `Download v${latestVersion.version}`}
            </button>
          )}
        </div>
      ...
      <div className="panel-section">
        ...
        {bundles.length === 0 ? (
          ...
          <ul className="bundle-list">
            {bundles.map(bundle => (
              <li key={bundle.bundleId} className="bundle-item">
        ...
        <div className="button-group">
          <button className="secondary" onClick={cleanupOldBundles}>
            Cleanup Old Bundles
          </button>
          <button className="danger" onClick={resetToOriginal}>
            Reset to Original
          </button>
          <button className="secondary" onClick={() => updateService.reloadApp()}>
            Reload App
          </button>
        </div>
      </div>
