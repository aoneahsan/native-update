...
app.post('/api/v1/bundles', upload.single('file'), async (req, res) => {
  ...
  const { version, channel = 'production', notes, mandatory, minNativeVersion } = req.body;
  ...
  try {
    ...
    if (process.env.SIGNING_PRIVATE_KEY_PATH) {
      try {
        const privateKey = await fs.readFile(process.env.SIGNING_PRIVATE_KEY_PATH, 'utf8');
